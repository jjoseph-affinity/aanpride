...
curl_setopt_array($curl0, array(
  ...
  CURLOPT_POSTFIELDS => "{
			...
			\"pinned\": false,
			\"draft\": false,
			\"body\": \"&#128308;  Sent away/Asked to leave $AttendeeName @ $SeminarName (via SeminarBoard).\"
...
$response0 = curl_exec($curl0);
	...
	case 201:
          echo "<span class='blacktext'>OK-->Sent Away/Asked to leave in RT; [$a tries]</span><br>";
      break;
    default:
      echo "<span class='blacktext'>ERROR-->nothing noted in RT [$a tries]--></span>" . $http_code0;
  }
    $a = $a + 1;
    sleep($b);
    $b = $b * 2;
