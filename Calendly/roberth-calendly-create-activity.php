...
echo $StartTime . '<br><br>';

$RobertAuthKey = "Authorization:  Userkeyauth YWJkZTJiNjYtNmI1NS00Nzk3LWJiZDEtNDk2NTcxMGFkYmE5Ojk2OTYyRkExLUIwMjAtNDI5My1CREEyLUVEREUzNEU2NDlBNA==";
...
$DescriptionAndComments = $Description . '<br><br>' . $Comments;
        ...
        echo 'starttime is: ' . $StartTime . "<br><br>";
        echo 'endtimeis is: ' . $EndTime . "<br><br>";
        echo 'title is: ' . $Title . "<br><br>";
        echo 'description and comments is: ' . $DescriptionAndComments . "<br><br>";
        echo 'Location is: ' . $Location . "<br><br>";
       // echo 'RT ID is: ' . $RedTailID . "<br>";

            // ASSIGN CONTACT (AND LINK CONTACT) TO ACTIVITY ON Robert's CALENDAR
/*
$a = 1;
$b = 1;
$http_code3_5 = '';
...
*/
            ...
            // ASSIGN CONTACT (But do NOT link ANY contacts) TO ACTIVITY ON ROBERT'S CALENDAR

//$StartTime = "2024-02-26T21:00:00.000000Z";
//$EndTime = "2024-02-26T22:00:00.000000Z";
...
$curl4 = '';
$response4 = '';
while ($a <= 6 && $http_code4 !== 201 /*&& ($RedTailID === '' || $RedTailID === NULL) */) {
  ...
  CURLOPT_POSTFIELDS => "{
        ...
        \"start_date\": \"{$StartTime}\",
        \"end_date\": \"{$EndTime}\",
        \"subject\": \"Exclusive Webinar Offer w/ {$Title}\",
  ...
  switch ($http_code4 = curl_getinfo($curl4, CURLINFO_RESPONSE_CODE)) {
	case 201:
          echo "OK activity created [$a tries]";
          $Log = "OK - activity created. [$a tries]\n";
      break;
    default:
      echo "ERROR activity NOT created! [$a tries]-->" . $http_code4 . "";
          $Log = "ERROR code $http_code4!\n";
  }
    $a = $a + 1;
    sleep($b);
    $b = $b * 2;
...
$ActivityID = $data4->activity->id;   
     echo "ID=" . $ActivityID       . "<br><br>";
}


	

                    // SET BODY EMAIL FOR ACTIVITY CREATED AND EMAIL ROBERT AND I

if ($Email !== '' && isset($Email)) {
    $DisplayStartTime = substr($StartTime, 0, 16);
	$Body = "
			<body>
			<p>RT Activity Created: {$Name1} via automation <i>4.0</i>.</p>
			</br>
...
while ($a <= 6 && $Sent === 'no') {
    ...
    //$mail->addBCC('rhall@affinityadvisorynetwork.com', 'Robert');
	$mail->isHTML(true);                               // Set email format to HTML
	$mail->Subject = "NEW CALENDLY-->RT ACTIVITY SCHEDULED (Robert): $Name1";
	$mail->Body    = $Body;
...
$Logfile = fopen("roberth-calendly-create-activity.txt", "a") or die("Unable to open file!");
fwrite($Logfile, $CurrentDate . "  " . $Log);
fclose($Logfile);
